...
export default function ChessAcademy({ user = null, onSignOut }) {
  ...
  function startGenericDrag(e, sq, piece, dropHandler, isFlipped=false){
    ...
    // Don't store a boardEl ref here — Board is defined inside ChessAcademy so
    // it gets unmounted/remounted on every re-render (e.g. when setSel fires),
    // which would make any DOM reference stale by the time onDragEnd runs.
  ...
  function onDragEnd(e){
    ...
    // Re-query the board at drop time — the stored ref would be stale because
    // Board is defined inside ChessAcademy and React unmounts+remounts it on
    // every re-render (setSel triggers one at drag start).
